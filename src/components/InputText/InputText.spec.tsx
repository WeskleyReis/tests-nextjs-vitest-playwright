...
describe('<InputText />', () => {
  describe('comportamento padrão', () => {
    ...
    test('usa labelText como arial-label quando possível', async () => {
      expect(input()).toHaveAttribute('aria-label', 'label')
    })

    test('usa placeholder como fallback de aria-label', async () => {
      expect(input({ labelText: undefined }))
      .toHaveAttribute('aria-label', 'placeholder')
    })
